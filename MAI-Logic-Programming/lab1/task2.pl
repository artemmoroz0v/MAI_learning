...
% First: Get the table of groups and the average score for each of the groups.
% Solution:
set_group(G, Result):-findall(X, student(G, X), Result).
set_table([G]):-findall(X, student(G, X), L), asserta(set_group(G, L)), write(G), write(": "), write(L), !.
amount_of_grades(Result, N, G):-student(G, N),grade(N,_,Result).
average_grade_in_group(G, Result):-bagof(X, amount_of_grades(2, X, G), Grade2), bagof(X, amount_of_grades(3, X, G),Grade3), bagof(X, amount_of_grades(4, X, G),Grade4), bagof(X, amount_of_grades(5, X, G), Grade5),
length(Grade2, Grade2_amount), length(Grade3, Grade3_amount), length(Grade4, Grade4_amount), length(Grade5, Grade5_amount), 
Grades is Grade2_amount + Grade3_amount + Grade4_amount + Grade5_amount, Result is (Grade2_amount * 2 + Grade3_amount * 3 + Grade4_amount * 4 + Grade5_amount * 5) / Grades.
%?-average_grade_in_group(102, Result).

%Second: For each subject, get a list of students who failed the exam (grade = 2).
%Solution:
subjects_with_bad_marks(Subject, Res):-bagof(X, grade(X, Subject, 2),Res).
%?-subjects_with_bad_marks('INF', Res), write(Res).

%Third: Find the number of failing students in each of the groups.
%Solution:
definition_of_fail_student(Name, GroupName):-grade(Name,_,2), student(GroupName, Name).
amount_of_fail_students(Group, Result):-setof(X, definition_of_fail_student(X, Group), Ans), length(Ans, Result).
%?-amount_of_fail_students(103, Res), write(Res).
